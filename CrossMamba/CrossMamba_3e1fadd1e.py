...
# Description: 实现了crossmamba.drawio文件中第二幅总体结构图
# Calling Attention:
"""
...
class VFEFM(nn.Module, PyTorchModelHubMixin):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
                 depths=[2, 2, 4, 2],
                 dims=[128, 256, 512, 1024],  # 每个阶段的通道数
                 d_state=128, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False,
                 cat_method='none',  # 控制CrossMamba输入融合策略
                 **kwargs):
        super().__init__()
        self.pre_fusion_stages = 3
        ...
        if self.ape:
            ...
            for i_layer in range(self.pre_fusion_stages):
                layer1 = FELayer(
                    dim=dims[i_layer],
        ...
        # 中期融合模块（CrossMamba）
        self.fusion = CrossMamba(d_model=dims[-2], dropout=attn_drop_rate)

        # CrossMamba 输出后的下采样模块（融合后统一使用）
        self.downsample = PatchMerging2D(dim=dims[2], norm_layer=norm_layer)

        # 融合后的统一特征提取模块（替代原来的 layer1[-1] 和 layer2[-1]）
        self.fused_layer = FELayer(
            dim=dims[3],
            depth=1,  # 可以设置为更深的堆叠
            d_state=d_state,
            drop=drop_rate,
    ...
    def forward_backbone(self, x1, x2):
        ...
        # 各自进入前3层特征提取
        for layer in self.layers1[:self.pre_fusion_stages]:
            x1 = layer(x1)
        for layer in self.layers2[:self.pre_fusion_stages]:
            x2 = layer(x2)

        # 设置CrossMamba的上下文融合输入
        x2_cat_x1 = x2
        x1_cat_x2 = x1
        ...
        # CrossMamba交互融合：输出两个分支结果
        x1_f, x2_f = self.fusion(x1, x2, x2_cat_x1, x1_cat_x2)

        # 融合两个分支后统一处理
        x_fused = x1_f + x2_f                      # 融合两个输出
        x_fused = self.downsample(x_fused)         # 下采样
        x = self.fused_layer(x_fused)              # 统一特征提取

        return x

    def forward(self, x1, x2):
        x = self.forward_backbone(x1, x2)          # 主干部分
        x = x.permute(0, 3, 1, 2)                  # (B, H, W, C) → (B, C, H, W)
        x = self.avgpool(x)
        x = torch.flatten(x, start_dim=1)
        x = self.head(x)                           # MLP/KAN
        return x
