# =============================================================================
# Name: CrossMamba_4ef
# Description: 实现了crossmamba.drawio文件中第三幅总体结构图
# Calling Attention:
"""
...
# --------------------- Overall Structure ----------------------
class Layer(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layer = Layer(
                ...
                norm_layer=norm_layer,
                downsample=PatchMerging2D if (i_layer < self.num_layers - 1) else None,  # 原为只有最后一层不含下采样
                use_checkpoint=use_checkpoint,
            )
            self.layers.append(layer)

        self.before_head_proj = nn.Linear(dims[-1]*2, dims[-1])
