...
class SS2D_with_SSD(nn.Module, PyTorchModelHubMixin):
    ...
    def in_projection(self, x: torch.Tensor, d_in_projection):
        b, h, w, c = x.shape  # 批次数，通道数，高，宽
        assert c == self.d_model, "in channel error"

        x_f = x.view(-1, c)
        bhw, _ = x_f.shape
        assert bhw == b * h * w, "x_f shape error"

        kan = eKANLinear(in_features=self.d_model,
                         out_features=d_in_projection)
        kan.to(device=device)

        y = kan(x_f)
        y_out = y.view(b, h, w, d_in_projection)
        #y_b, y_h, y_w, y_c = y.shape
        #assert y_b == b and y_h == h and y_w == w and y_c == d_in_projection, "back shape error"
    ...
    def forward(self, u: torch.Tensor, seqlen=None, seq_idx=None, cu_seqlens=None):
        ...
        zxbcdt = self.in_projection(u, d_in_projection=self.d_in_proj)
